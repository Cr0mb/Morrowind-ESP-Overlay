...
def get_client_area(window_title):
    ...
    client_top = rect.top + border_y - ctypes.windll.user32.GetSystemMetrics(4)
...
class Overlay(QtWidgets.QWidget):
    def __init__(self, game_window, pm, base_address, *args, **kwargs):
        ...
        self.show_npc_names = True
        self.show_health_bars = True
        self.show_health_values = True
        self.show_entity_lines = True  # New flag for entity line ESP
        self.show_npc_lines = True     # New flag for NPC line ESP
    ...
    def paintEvent(self, event):
        ...
        for entity in self.entities:
            ...
            if screen_pos:
                ...
                if self.show_entity_names and entity["health"] is None:
                    painter.setPen(QtGui.QColor(255, 255, 255, 255))
                    painter.drawText(head_position + QtCore.QPointF(10, -15), entity["name"])

                if self.show_npc_names and entity["health"] is not None:
                    painter.setPen(QtGui.QColor(255, 255, 255, 255))
                    painter.drawText(head_position + QtCore.QPointF(10, -15), entity["name"])
                ...
                # Draw lines to entities or NPCs based on the toggles
                if (self.show_entity_lines and entity["health"] is None) or (self.show_npc_lines and entity["health"] is not None):
                    line_end = screen_pos
                    line_start = QtCore.QPointF(self.width() / 2, self.height())  # Bottom center of the screen
                    painter.setPen(QtGui.QColor(255, 0, 0, 255))  # Red color for lines
    ...
    def toggle_entity_lines(self):
        self.show_entity_lines = not self.show_entity_lines

    def toggle_npc_lines(self):
        self.show_npc_lines = not self.show_npc_lines
...
def show_gui(overlay):
    ...
    def toggle_entity_lines():
        overlay.toggle_entity_lines()

    def toggle_npc_lines():
        overlay.toggle_npc_lines()

    root = tk.Tk()
    root.title("ESP Features")

    ttk.Checkbutton(root, text="Show Entity Names", command=toggle_entity_names).pack()
    ttk.Checkbutton(root, text="Show NPC Names", command=toggle_npc_names).pack()
    ...
    ttk.Checkbutton(root, text="Show Entity Line ESP", command=toggle_entity_lines).pack()  # New checkbox for entity lines
    ttk.Checkbutton(root, text="Show NPC Line ESP", command=toggle_npc_lines).pack()  # New checkbox for NPC lines
